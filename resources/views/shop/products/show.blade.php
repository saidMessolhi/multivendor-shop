    ...
    <div class="max-w-7xl mx-auto px-4 py-10">
        ...
        <nav class="flex items-center gap-2 text-sm text-slate-400 mb-8">
            ...
            @if ($product->category)
                ...
                <a href="{{ route('category.show', $product->category) }}"
                    class="hover:text-slate-600">{{ $product->category->name }}</a>
            @endif
            <span>/</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
                    ...
                    @if ($product->images && count($product->images))
                        @foreach ($product->images as $i => $img)
                            <img src="{{ asset('storage/' . $img) }}" alt="{{ $product->name }}"
                                x-show="active === {{ $i }}" class="w-full h-full object-cover">
                        @endforeach
                    @else
            ...
            <div>
                ...
                <h1 class="text-3xl font-extrabold text-slate-900 mt-2 mb-3 leading-tight"
                ...
                @if ($product->rating_count > 0)
                    <div class="flex items-center gap-2 mb-4">
                        <div class="flex">
                            @for ($i = 1; $i <= 5; $i++)
                                <svg class="w-4 h-4 {{ $i <= round($product->rating_avg) ? 'text-yellow-400' : 'text-gray-200' }}"
                        ...
                        <span
                            class="text-sm font-semibold text-slate-700">{{ number_format($product->rating_avg, 1) }}</span>
                ...
                <div class="flex items-baseline gap-3 mb-6">
                    <span
                        class="text-4xl font-extrabold text-slate-900">${{ number_format($product->current_price, 2) }}</span>
                    @if ($product->isOnSale())
                ...
                <div class="border-t border-gray-100 pt-5 space-y-2 text-sm text-slate-500">
                    @if ($product->sku)
                        <div class="flex gap-2"><span
                                class="font-medium text-slate-700 w-20">SKU:</span>{{ $product->sku }}</div>
                    @endif
                    @if ($product->category)
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-8 mb-8">
            ...
            @auth
                ...
                <form method="POST" action="{{ route('reviews.store', $product) }}" class="mb-8 p-5 bg-gray-50 rounded-xl"
                    x-data="{ rating: 0 }">
                    @csrf
                    ...
                    <button type="submit"
                        class="px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-xl text-sm transition-colors">
                        Submit Review
                    </button>
                </form>
            ...
            <div class="space-y-5">
                @forelse($product->reviews->where('is_approved', true) as $review)
                    <div class="border-b border-gray-50 pb-5 last:border-0">
                        ...
                        @if ($review->title)
                            <p class="font-semibold text-slate-800 text-sm mb-1">{{ $review->title }}</p>
                        @endif
                        @if ($review->body)
                ...
                @empty
                    <p class="text-slate-400 text-sm text-center py-6">No reviews yet. Be the first to review this product!
                    </p>
                @endforelse
            </div>
        </div>
        ...
        @if ($related->count())
            <div>
                <h2 class="text-xl font-bold text-slate-900 mb-5" style="font-family:'Syne',sans-serif;">Related Products
                </h2>
